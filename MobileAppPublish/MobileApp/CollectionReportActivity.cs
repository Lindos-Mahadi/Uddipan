...
namespace PMS.Droid
{
    [Activity(Label = "Collections", Theme = "@style/Theme.DesignDemo", ScreenOrientation = Android.Content.PM.ScreenOrientation.Portrait)]
    public class CollectionReportActivity : BaseActivity
    {
        private List<CenterModel> centers = null;
        protected override void OnCreate(Bundle savedInstanceState)
        {
            base.OnCreate(savedInstanceState);
            SetContentView(Resource.Layout.CollectionReport);
            ...
            try
            {
               // LoadCollectionList();
                ...
                if (user != null && !string.IsNullOrEmpty(user.InstallmentDate))
                {
                    ...
                    {                       
                        Title = "Collections - " + dt.ToString("MMM dd, yyyy");
                    }
                }
                SetCenterSpinners();
        ...
        private void LoadCollectionList(int centerID)
        {
            ...
            var listItsms = dbVals.GetMemberWiseCollectionSummary(centerID);
            var lv = FindViewById<ListView>(Resource.Id.collectionSummaryList);
            lv.Adapter = new CollectionReportGridAdapter(this, listItsms.OrderBy(o => o.RecordType).ThenBy(b => b.MemberName).ToList(), centerID);
        }
        private void SetCenterSpinners()
        {
            ...
            if (default(int) != officeId)
            {
                ...
                var lst = syncHelper.GetCentersByOffice(officeId, 0);
                centers = lst;
                var spnCenter = FindViewById<Spinner>(Resource.Id.spnCenterCollection);
